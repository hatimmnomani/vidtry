...
import type { Dispose, JSX, ReadSignal, Scope, SignalOptions, Signals, WriteSignal, WriteSignals } from '../runtime';
...
export type CustomElementPropDefinition<Value = unknown> = SignalOptions<Value> & (Value extends undefined ? {} : {
    /**
...
export type CustomElementSetup<T extends AnyCustomElement> = (instance: CustomElementInstance<T>) => InferCustomElementMembers<T> extends Record<any, never> ? void | (() => JSX.Element) : InferCustomElementMembers<T> & {
    $render?: () => JSX.Element;
};
export interface CustomElementDefinition<T extends AnyCustomElement = AnyCustomElement> extends Omit<CustomElementDeclaration<T>, 'setup'> {
    /** @internal */
    setup: (instance: NonNullable<T['instance']>) => InferCustomElementMembers<T> & {
        $render?(): JSX.Element;
    };
}
export type InferCustomElement<T> = T extends CustomElementDefinition<infer Element> ? Element : never;
export interface AnyCustomElement extends HTMLCustomElement<any, any, any> {
}
export interface HTMLCustomElementConstructor<T extends AnyCustomElement = AnyCustomElement> extends Constructor<T> {
    readonly observedAttributes: string[];
}
export interface HTMLCustomElement<Props = {}, Events = {}, CSSVars = {}> extends HTMLElement, HostElement {
    ...
    /** @internal only holds type - not a real prop. */
    ___events?: Events;
    addEventListener<K extends keyof Events>(type: K, listener: (this: Element, ev: Events[K]) => any, options?: boolean | AddEventListenerOptions): void;
    ...
    removeEventListener<K extends keyof Events>(type: K, listener: (this: Element, ev: Events[K]) => any, options?: boolean | EventListenerOptions): void;
...
export type InferCustomElementProps<T> = T extends HTMLCustomElement<infer Props, any, any> ? Props : never;
export type InferCustomElementEvents<T> = T extends HTMLCustomElement<any, infer Events, any> ? Events : never;
export type InferCustomElementCSSProps<T> = T extends HTMLCustomElement<any, any, infer CSSVars> ? CSSVars : never;
...
export type InferCustomElementMembers<T> = T extends AnyCustomElement ? Simplify<Omit<T, keyof AnyCustomElement | '$render'>> : never;
export interface CustomElementInstanceInit<Props = {}> {
    scope?: Scope | null;
    props?: Readonly<Partial<Props>>;
}
export interface CustomElementInstance<T extends AnyCustomElement = AnyCustomElement> extends ElementLifecycleManager {
    /** @internal */
    [SCOPE]: Scope | null;
    /** @internal */
...
export type PartialCustomElementDeclaration<T extends AnyCustomElement> = InferCustomElementMembers<T> extends EmptyRecord ? Omit<CustomElementDeclaration<T>, (InferCustomElementProps<T> extends Record<string, never> ? 'props' : '') | 'setup'> & {
    setup?: CustomElementDeclaration<T>['setup'];
} : InferCustomElementProps<T> extends Record<string, never> ? Omit<CustomElementDeclaration<T>, 'props'> : CustomElementDeclaration<T>;
