import { createComment, isDOMNode, createFragment, attachDeclarativeShadowDOM, setAttribute, toggleClass, setStyle, listenEvent, camelToKebabCase, trimTrailingSemicolon, flattenArray, runAll, isDOMElement } from './chunk-JJON6UQL.js';
import { createElementInstance } from './chunk-HXFROLDL.js';
import { getCustomElementInstance, RENDER, SCOPE, PROPS, CONNECT, MEMBERS, ATTACH, HOST } from './chunk-BKACTARQ.js';
import { root, isFunction, isArray, isString, isNumber, unwrapDeep, effect, isUndefined, onDispose, scoped, peek, computed, isNull, isBoolean, noop } from './chunk-D2IDN6VA.js';
import { root as root$1, tick, scoped as scoped$1, getScope, isFunction as isFunction$1, effect as effect$1, untrack } from '@maverick-js/signals';
...
function $$_next_custom_element(definition, walker = hydration?.w) {
...
function createHTMLElement(definition, init) {
  ...
  }, _a3._definition = definition, _a3._init = init, _a3._attrToProp = attrToProp, _a3._propToAttr = propToAttr, _a3;
}
...
var HTMLCustomElement = class extends HTML_ELEMENT {
  ...
  disconnectedCallback() {
    ...
    for (const disconnectCallback of this._disconnectCallbacks) {
      scoped$1(disconnectCallback, this._connectScope);
    }
    this._connectScope = null;
    tick();
  ...
  attachComponent(instance) {
    const ctor = this.constructor, definition = ctor._definition, init = ctor._init;
    if (this._instance) {
    ...
    if (definition.css && !init?.adoptCSS) {
      console.warn(
    ...
    if (!hydration && definition.shadowRoot && definition.css && init?.adoptCSS) {
      init.adoptCSS(this._root, definition.css);
    }
    ...
    for (const attachCallback of [...instance[ATTACH], ...this._attachCallbacks]) {
      scoped$1(attachCallback, instance[SCOPE]);
    }
  ...
  async _setup() {
    ...
    const { setup } = await import('./setup-JDTOOFMN.js');
    await setup(this);
    this._pendingSetup = false;
  }
};
...
function $registerCustomElement(definition, init) {
...
function $$_custom_element(definition, props, spreads) {
  ...
  const hasInnerHTML = !!props?.innerHTML, innerHTML = hasInnerHTML ? resolve(props.innerHTML) : null, children = hasInnerHTML ? innerHTML : resolve(props?.$children), instance = createElementInstance(definition, { props });
