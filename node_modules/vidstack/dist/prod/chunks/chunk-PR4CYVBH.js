import { VideoProvider } from './chunk-2X6WFYTF.js';
import { createRAFLoop } from './chunk-F6EZ2TY4.js';
import { HLS_LISTENERS, coerceToError } from './chunk-WQM6HX25.js';
import { preconnect, loadScript } from './chunk-EWETREDA.js';
import { isHLSSupported } from './chunk-LNPU55YF.js';
...
var HLSProvider = class extends VideoProvider {
  constructor() {
    ...
    this.k = `${JS_DELIVR_CDN}/npm/hls.js@^1.0.0/dist/hls${".min.js"}`;
    this.config = {};
  }
  get type() {
    return "hls";
  ...
  setup(context) {
    super.setup(context);
    loadHLSLibrary(this.k, context).then((ctor) => this.s.set(() => ctor));
    useHLS(this, this.config, this.s, this.n, context, this.t);
  }
  get ctor() {
    return this.s();
  }
  ...
  async loadSource({ src }) {
    effect(() => {
      if (!isString(src))
        return;
      const instance = this.n();
      instance?.loadSource(src);
    });
  }
  onInstance(callback) {
    const instance = peek(this.n);
    if (instance)
      callback(instance);
    this.t.add(callback);
